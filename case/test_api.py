...
class Test_api(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # 如果有登录的话，就在这里先登录了
        wirteexcel.copy_excel(testxlsx, reportxlsx)  # 复制xlsx

    @ddt.data(*testdata)
    def test_api(self, data):
        # 先复制excel数据到report
        res = api.send_requests(self.s, data)

        api.wirte_result(res, filename=reportxlsx)
        # 检查点 checkpoint
